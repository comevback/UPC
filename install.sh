...
if [ "$os_name" = "Linux" ]; then
    sed -i "s/const CENTRAL_SERVER = process.env.CENTRAL_SERVER || '.*';/const CENTRAL_SERVER = 'http:\/\/${ip_address}:8000';/" $backend_method
    sed -i "s/const hostURL = process.env.HOST_URL || '.*';/const hostURL = 'http:\/\/${ip_address}:4000';/" $backend_method
elif [ "$os_name" = "Darwin" ]; then
    sed -i '' "s/const CENTRAL_SERVER = process.env.CENTRAL_SERVER || '.*';/const CENTRAL_SERVER = 'http:\/\/${ip_address}:8000';/" $backend_method
    sed -i '' "s/const hostURL = process.env.HOST_URL || '.*';/const hostURL = 'http:\/\/${ip_address}:4000';/" $backend_method
fi